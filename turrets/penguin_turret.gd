...
func _ready():
	attack_speed_rate = 150
	tower_dmg = 1
	_movement_animation = $penguin/AnimationPlayer

	
func follow_target(target, delta):
	var target_vector = $penguin.global_position.direction_to(
	...
	$penguin.basis = $penguin.basis.slerp(target_basis, _slerp_progress)
	_slerp_progress += delta*8
...
func _fire_projectile_if_possible():
	...
	if Time.get_ticks_msec() > (_latest_projectile_fired + attack_speed_rate) and not _waiting_for_projectile:
		...
		_movement_animation.play("Animation")
	if Time.get_ticks_msec() > (_latest_animation + 200) and _waiting_for_projectile:
		...
		projectile.starting_position = $penguin/projectile_spawn.global_position
